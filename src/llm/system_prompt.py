...
def build_system_prompt_core(agent: Any, sender_name: str = None) -> str:
    ...
    # 杜绝「由徐宇坤(IT工程师)评估后走正规」「通过钉钉→工作台→申请」这类漏网。
    prompt += (
    ...
    prompt += (
        ...
        "知识库查不到时，直接说「知识库中未找到相关信息」，严禁凭借训练数据编造。"
        ...
        "若知识库中确实无该信息，说「知识库中未找到相关信息」，绝不顺着用户的意思确认。"
    ...
    prompt += (
        ...
        f"markdown{md_note};"
        "不确定查工具;查到标来源;"
        "【查不到就说没查到】如果知识库搜索(kb_search)无结果或结果不包含答案，"
        "必须明确告知用户'知识库中未找到相关信息'，严禁编造任何地址/IP/流程/配置。"
